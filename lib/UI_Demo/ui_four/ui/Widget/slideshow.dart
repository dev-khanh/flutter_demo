...
class _SlideShowViewState extends State<SlideShowView> {
  ...
  Widget build(BuildContext context) {
    movieListBloc.fetchMovieList('now_playing');
    return StreamBuilder(stream: movieListBloc.movieList ,builder: (context, AsyncSnapshot<Item_Model> snapshot){
      if (snapshot.hasData) {
  ...
  Widget buildContent(AsyncSnapshot<Item_Model> snapshot, BuildContext context){
    ...
    return Padding(padding: const EdgeInsets.only(top: 20.0),
      child: CarouselSlider(height: width / 2, aspectRatio: 16 / 9, viewportFraction: 0.8, initialPage: 0, enableInfiniteScroll: true, reverse: false, autoPlay: true, autoPlayInterval: Duration(seconds: 3),
        autoPlayAnimationDuration: Duration(milliseconds: 800), pauseAutoPlayOnTouch: Duration(seconds: 10), enlargeCenterPage: true,
