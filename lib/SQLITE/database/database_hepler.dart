...
class DatabaseHelper {
  ...
  Future<List<User>> getUser() async {
    ...
    for (int i = 0; i < list.length; i++) {
      print('DEVK LIST name: '+ list[i]["lastname"].toString());
//      print('DEVK LIST: $i');
      var user = new User(list[i]["firstname"], list[i]["lastname"], list[i]["dob"]);
  ...
  Future<bool> update(User user) async {
    var dbClient = await db;
    int res =   await dbClient.update("User", user.toMap(), where: "id = ?", whereArgs: <int>[user.id]);
